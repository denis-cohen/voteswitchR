...
aggregate_switches <- function(data_file,
  ...
  if (not(class(data_file) == "voteswitchr_data_file")) {
    ...
    for (j in jj) {
      for (m in 1:length(data_file$data_imp[[j]])) {
        ...
        data_file$data_imp[[j]][[m]]$out_vote <-
          factor(data_file$data_imp[[j]][[m]]$vote, labels = out_names[vt_in])
        data_file$data_imp[[j]][[m]]$out_l_vote <-
          factor(data_file$data_imp[[j]][[m]]$l_vote, labels = out_names[l_vt_in])
      }
    }
