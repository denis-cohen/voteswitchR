...
#'
#' @param data A data object generated via [build_data_file()]
#' and processed via [recode_switches()]. Can be a single
#' \code{tbl_df}/\code{tbl}/\code{data.frame} or, in the case multiply imputed
#' vote switching counts, a list thereof.
#' @param y_names A vector of column names for vote switching counts, as
#' generated by [recode_switches].
#' @param baseline An element of \code{y_names} that determines the baseline
#' category for the MAVCL model. It must describe a column with no deterministically
#' empty cells. If \code{NULL}, the option defaults to the last element of
#' \code{y_names}.
#' @param null_model Logical; if \code{TRUE}, the function runs a null model
...
#' via \code{data}.
#' @param random_slopes Logical; if \code{TRUE}, the slopes for the main
...
#' @param parallelize Logical; if \code{TRUE}, estimation is not only
...
run_mavcl <- function(data,
  ...
  ## Set baseline
  if (is.null(baseline)) {
    baseline <- y_names[length(y_names)]
    message(paste0("No baseline provided. Setting baseline to '", baseline, "'."))
  } else {
    if (!(baseline %in% y_names)) {
      stop(paste0(
        "'baseline = ",
        baseline,
        "' is not in 'y_names'. ",
  ...
  ## Reorder y_names
  y_names <- c(y_names[y_names != baseline], baseline)
  y_check <- as.matrix(data[[1]][y_names]) %>%
    apply(2, function (y)
      sum(y == -1))

  if (y_check[baseline] > 0) {
    stop(
      paste0(
        "'baseline = ",
        baseline,
        ...
        paste(y_names[y_check == 0], collapse = ", "),
        "."
      )
    )
  }
