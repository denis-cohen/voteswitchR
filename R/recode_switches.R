...
#' \code{\link{voteswitchR::build_data_file()}}.
#' @param subgroup A character that specifies the column-name of electorate subgroups.
#' Should be \code{NULL} (the default) if no subgroup analyses are required.
#' @param stack_by_subgroup Logical; if \code{TRUE}, subgroups are stacked into long format
#' such that switching proportions from the MAVCL model sum to one within subgroups.
#' If \code{FALSE}, subgroup-specific switching is recorded in wide format such that
#' switching proportions for each subgroup sum to the size of the subgroup.
#' Only takes effect if \code{subgroup != NULL}.
#' @param assign_others Optional: A character that specifies a level for residual
#' other parties for which \code{switch_factor} information is not available.
#' @param mappings A copy of \code{\link{voteswitchR::mappings}}, possibly
...
#' that specifies the marginal categories of the desired voter transition matrix.
  ...
  if (is.null(subgroup)) {
    if (type == "elections") {
      ...
      y_structure <- switches %>%
        ...
        dplyr::mutate(dyad = dplyr::case_when(type == "loss" ~ to,
                                              type == "gain" ~ from,
                                              TRUE ~ NA_character_)) %>%
        dplyr::select(-from, -to) %>%
        split(., .$cat) %>%
    ...
    if (stack_by_subgroup) {
      if (type == "elections") {
        ...
        elec_switches <- switches %>%
          dplyr::mutate(elec_group_id = paste(elec_id,!!as.name(subgroup), sep = "-")) %>%
      ...
      if (type == "elections") {
        ...
        y_structure <- switches %>%
          ...
          dplyr::mutate(switch = paste(from, to,!!as.name(subgroup), sep = "_")) %>%
          dplyr::mutate(
            type = dplyr::case_when(
              cat == from & cat == to ~ "retain",
              cat == from & cat != to ~ "loss",
