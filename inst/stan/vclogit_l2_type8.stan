...
data {
  ...
  int<lower=1> num_elections;
  int<lower=1> election_id[J];
  int<lower=1> num_countries;
  int<lower=1> country_id[J];
}
...
parameters {
  ...
  vector<lower=0>[ncatm1] sigma_elections;
  matrix[ncatm1,num_elections] nu_raw_elections;
  cholesky_factor_corr[ncatm1] L_elections;
  vector<lower=0>[ncatm1] sigma_countries;
  matrix[ncatm1,num_countries] nu_raw_countries;
  cholesky_factor_corr[ncatm1] L_countries;
}

transformed parameters {
  matrix[J,Q] nu;
  matrix[num_parties,ncatm1] nu_parties;
  matrix[num_elections,ncatm1] nu_elections;
  matrix[num_countries,ncatm1] nu_countries;
  
  nu = scale_r_cor(nu_raw, sigma, L);
  nu_parties = scale_r_cor(nu_raw_parties, sigma_parties, L_parties);
  nu_elections = scale_r_cor(nu_raw_elections, sigma_elections, L_elections);
  nu_countries = scale_r_cor(nu_raw_countries, sigma_countries, L_countries);
}
...
model {
  ...
  for (c in 1:ncatm1) {
    log_prob[c, ] =  beta[c] * X_transpose + 
      ...
      for (j in 1:J) {
        log_prob[c,j] = log_prob[c,j] + 
          nu_parties[party_id[j],c] +
          nu_elections[election_id[j],c] +
          nu_countries[country_id[j],c];
      }
  ...
  target += std_normal_lpdf(to_vector(nu_raw_elections));
  target += student_t_lpdf(sigma_elections | 3, 0, 2.5)
    - 4 * student_t_lccdf(0 | 3, 0, 2.5);
  target += lkj_corr_cholesky_lpdf(L_elections | 2);
  
  target += std_normal_lpdf(to_vector(nu_raw_countries));
  target += student_t_lpdf(sigma_countries | 3, 0, 2.5)
    - 4 * student_t_lccdf(0 | 3, 0, 2.5);
  target += lkj_corr_cholesky_lpdf(L_countries | 2);
...
generated quantities {
  ...
  matrix[ncatm1,ncatm1] Sigma_elections = quad_form_diag(tcrossprod(L_elections), sigma_elections);
  matrix[ncatm1,ncatm1] Sigma_countries = quad_form_diag(tcrossprod(L_countries), sigma_countries);
}
