...
date: "This version: March 25, 2024"
bibliography: 
  - fig/references.bib
  - fig/sup.bib
---
...
The current version, `voteswitchR 0.3.0`, was released on March 25, 2024. It incorporates the following changes and additions:
...
1. `switches_imp`: 25 imputations of the cell counts for the raw, idiosyncratic transition matrices for all elections included in `voteswitchR`.
...
1. `raked_switches_imp`: 25 imputations of the rake-weighted cell counts for the raw, idiosyncratic transition matrices for all elections included in `voteswitchR`.
...
concepts <- data_file$data %>%
  ...
  dplyr::rename(
    ...
    `Party like-dislike scores` = like_1,
    `Left-right party placements` = lr_1,
    `Gender (binary)` = male,
    `Age` = age
  ) %>%
